...
def resolve_all_characters(_, info):
    ...
    try:
        characters = conn.execute("SELECT id, name, species, skill, home_planet_id FROM characters").fetchall()
...
def resolve_character(_, info, id):
    conn = get_db_connection()
    try:
        character = conn.execute("SELECT id, name, species, skill, home_planet_id FROM characters WHERE id = ?", (id,)).fetchone()
...
def resolve_all_starships(_, info):
    conn = get_db_connection()
    try:
        starships = conn.execute("SELECT id, name, model, manufacturer, specialist FROM starships").fetchall()
...
def resolve_starship(_, info, id):
    conn = get_db_connection()
    try:
        starship = conn.execute("SELECT id, name, model, manufacturer, specialist FROM starships WHERE id = ?", (id,)).fetchone()
...
def resolve_create_character(_, info, input):
    ...
    try:
        ...
        conn.execute(
            "INSERT INTO characters (name, species, skill, home_planet_id) VALUES (?, ?, ?, ?)",
            (input["name"], input.get("species"), input.get("skill"), input.get("homePlanetId")),
        )
        conn.commit()
        char_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
        character = conn.execute(
            "SELECT id, name, species, skill, home_planet_id FROM characters WHERE id = ?", (char_id,)
        ).fetchone()
...
def resolve_update_character(_, info, input):
    conn = get_db_connection()
    try:
        character = conn.execute("SELECT id, name, species, skill, home_planet_id FROM characters WHERE id = ?", (input["id"],)).fetchone()
        if not character:
            raise Exception(f"Karakter dengan ID {input['id']} tidak ditemukan.")
        conn.execute(
            "UPDATE characters SET name = ?, species = ?, skill = ?, home_planet_id = ? WHERE id = ?",
            (
                input.get("name", character["name"]),
                input.get("species", character["species"]),
                input.get("skill", character["skill"]),
                input.get("homePlanetId", character["home_planet_id"]),
                input["id"],
            ),
        )
        conn.commit()
        updated_character = conn.execute("SELECT id, name, species, skill, home_planet_id FROM characters WHERE id = ?", (input["id"],)).fetchone()
...
def resolve_delete_character(_, info, id):
    ...
    try:
        ...
        # Optional: Check if character has assigned starships and handle accordingly
...
def resolve_update_starship(_, info, input):
    conn = get_db_connection()
    try:
        starship = conn.execute("SELECT id, name, model, manufacturer, specialist FROM starships WHERE id = ?", (input["id"],)).fetchone()
        if not starship:
            raise Exception(f"Kapal dengan ID {input['id']} tidak ditemukan.")
        conn.execute(
            "UPDATE starships SET name = ?, model = ?, manufacturer = ?, specialist = ? WHERE id = ?",
            (
                input.get("name", starship["name"]),
                input.get("model", starship["model"]),
                input.get("manufacturer", starship["manufacturer"]),
                input.get("specialist", starship["specialist"]),
                input["id"],
            ),
        )
        conn.commit()
        updated_starship = conn.execute("SELECT id, name, model, manufacturer, specialist FROM starships WHERE id = ?", (input["id"],)).fetchone()
...
def resolve_delete_starship(_, info, id):
    ...
    try:
        ...
        # Optional: Check if starship is assigned to any characters and handle accordingly
...
def resolve_create_starship(_, info, input):
    ...
    try:
        conn.execute(
            "INSERT INTO starships (name, model, manufacturer, specialist) VALUES (?, ?, ?, ?)",
            (input["name"], input.get("model"), input.get("manufacturer"), input.get("specialist")),
        )
        conn.commit()
        starship_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
        starship = conn.execute(
            "SELECT id, name, model, manufacturer, specialist FROM starships WHERE id = ?", (starship_id,)
        ).fetchone()
